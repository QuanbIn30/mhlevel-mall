...
public class MyCommentsController  extends BaseController {
    ...
    @PostMapping("/pending")
    public MHLEVELJSONResult pending(
        ...
        if (myOrder.getIsComment() == YesOrNo.YES.getType()) {
            return MHLEVELJSONResult.errorMsg("该笔订单已经评价");
        }

        List<OrderItems> list = myCommentsService.queryPendingComment(orderId);

        return MHLEVELJSONResult.ok(list);
    }


    @ApiOperation(value = "保存评论列表", notes = "保存评论列表", httpMethod = "POST")
    @PostMapping("/saveList")
    public MHLEVELJSONResult saveList(
            @ApiParam(name = "userId", value = "用户id", required = true)
            @RequestParam String userId,
            @ApiParam(name = "orderId", value = "订单id", required = true)
            @RequestParam String orderId,
        ...
        if (commentList == null || commentList.isEmpty() || commentList.size() == 0) {
            return MHLEVELJSONResult.errorMsg("评论内容不能为空！");
        }
